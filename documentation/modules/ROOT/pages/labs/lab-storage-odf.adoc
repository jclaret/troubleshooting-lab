...
# Check PVC being resized
oc describe pvc upload-data | grep Resize
----
...
After activation, wait 5 minutes, then begin.
...
[NOTE]
====
In this deployment, ODF has been configured with 4 storage nodes, but only 3 nodes are actively running MON and OSD pods. The 4th node is available as a standby node for high availability but doesn't currently host any ODF services. This provides redundancy and allows for maintenance operations without affecting cluster availability.

To verify the ODF cluster configuration:

[.lines_space]
[.console-input]
[source,bash, subs="+macros,+attributes"]
----
# Check which nodes have ODF services running
oc get pods -n openshift-storage -o wide | grep -E "(rook-ceph-mon|rook-ceph-osd)" | awk '{print $1, $7}' | sort

# Verify MON pods are running on 3 nodes only
oc get pods -n openshift-storage -l app=rook-ceph-mon -o wide

# Verify OSD pods are running on 3 nodes only  
...
[source,bash, subs="+macros,+attributes"]
----
# Check storage pods in Pending
oc get po -n openshift-storage | egrep -v "Completed|Running"

# Check why MON/OSD pods are in Pending

oc describe pod rook-ceph-mon-XXXXX

  Warning  FailedScheduling  9s (x10 over 94s)  default-scheduler  0/8 nodes are available: 1 Insufficient cpu, 2 node(s) didn't match Pod's node affinity/selector, 2 node(s) didn't match pod anti-affinity rules, 3 node(s) had untolerated taint {node-role.kubernetes.io/master: }. preemption: 0/8 nodes are available: 3 node(s) didn't match pod anti-affinity rules, 5 Preemption is not helpful for scheduling.

oc describe pod rook-ceph-osd-XXXXX

  Warning  FailedScheduling  4m45s  default-scheduler  0/8 nodes are available: 3 node(s) had untolerated taint {node-role.kubernetes.io/master: }, 5 node(s) didn't match Pod's node affinity/selector. preemption: 0/8 nodes are available: 8 Preemption is not helpful for scheduling.
...
The ODF cluster will take approximately 5-10 minutes to return to HEALTH_OK status after applying the fix.
...
- Select the instance that is in **Stopped** state (remove filter "Instance state = running")  
...
- A PersistentVolumeClaim using storageclass ocs-storagecluster-cephfs and RWX for storing uploaded photos
...
# Checl application pods
oc get pod
NAME                              READY   STATUS              RESTARTS   AGE
image-uploader-745bb8fb7b-6gqtp   0/1     ContainerCreating   0          4m7s
image-uploader-745bb8fb7b-7pwp5   1/1     Running             0          4m7s
image-uploader-745bb8fb7b-qpgp2   0/1     ContainerCreating   0          4m7s


oc describe pod image-uploader-745bb8fb7b-qpgp2
  
  Warning  FailedAttachVolume  3m36s  attachdetach-controller  Multi-Attach error for volume "pvc-39a8389f-4319-4255-beda-43cd33043480" Volume is already used by pod(s) image-uploader-745bb8fb7b-7pwp5

----

**During the initial deployment, you will encounter 2 problems:**

- **AccessMode Issue**: With ReadWriteOnce, the PVC can be mounted read-write by only one node at a time. If replicas land on different nodes, the first node attaches the volume and the others stay Pending/ContainerCreating with a multi-attach/exclusive-access error until the AccessMode is changed to RWX
...
- **PVC Size Issue**: This will prevent file uploads with errors like this, the fix should be resize PVC from console or editing PVC and changing the size 
