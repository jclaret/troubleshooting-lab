...
cd /home/demo-user/troubleshooting-lab/lab-materials/00-storage-odf/lab1-node-labeling
...
Apply the fix to restore ODF node labeling:
...
# 1. Get the list of storage nodes
STORAGE_NODES=$(oc get nodes -l node-role.kubernetes.io/worker -o name | head -3)

# 2. Restore ODF storage labels
for node in $STORAGE_NODES; do
    oc label $node cluster.ocs.openshift.io/openshift-storage=""
    oc label $node node-role.kubernetes.io/worker=""
done

# 3. Monitor ODF cluster recovery
oc get storagecluster -n openshift-storage -w
----
====
...
[source,bash]
----
cd troubleshooting-lab/lab-materials/00-storage-odf/02-odf-file
...
[source,bash]
----
cd troubleshooting-lab/lab-materials/00-storage-odf/03-odf-block
...
[source,bash]
----
cd troubleshooting-lab/lab-materials/00-storage-odf/01-odf-s3
