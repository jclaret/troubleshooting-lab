...
* **Metal³**: Manages bare metal host provisioning and hardware discovery
...
**Best Practice**: Always start troubleshooting by locally validating your SiteConfig with `kustomize build --enable-alpha-plugins` before pushing to Git or investigating cluster-side issues.
...
=== Local SiteConfig Validation and CR Extraction

Before troubleshooting deployed resources, you can locally preview what CRs will be generated from a SiteConfig using the kustomize plugin:
...
# Clone the CNF features deploy repository for SiteConfig generator plugin
git clone https://github.com/openshift-kni/cnf-features-deploy.git
cd cnf-features-deploy/ztp/siteconfig-generator-kustomize-plugin

# Extract generated CRs from SiteConfig locally (preview mode)
kustomize build --enable-alpha-plugins /path/to/your/siteconfig/directory

# Save generated manifests for analysis
kustomize build --enable-alpha-plugins /path/to/your/siteconfig/directory > generated-resources.yaml

# Analyze specific resource types that will be created
kustomize build --enable-alpha-plugins /path/to/your/siteconfig/directory | grep -A 5 -B 5 "kind: ClusterDeployment"
kustomize build --enable-alpha-plugins /path/to/your/siteconfig/directory | grep -A 5 -B 5 "kind: AgentClusterInstall"
kustomize build --enable-alpha-plugins /path/to/your/siteconfig/directory | grep -A 5 -B 5 "kind: BareMetalHost"
----

This technique is invaluable for:

* **Pre-deployment Validation**: Verify SiteConfig syntax and generated resource correctness
* **Resource Debugging**: Identify malformed or missing resources before ArgoCD processing
* **Template Understanding**: See exact resource specifications that will be applied
* **Diff Analysis**: Compare expected vs. actual cluster state
...
- Metal³ operator component health and logs
====
...
3. **Component Interaction**: Understand how ArgoCD, ACM, and Metal³ interact
...
* link:https://metal3.io/documentation/[Metal³ Documentation]
