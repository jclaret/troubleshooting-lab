...
# Analyze out-of-sync resources if application is not synced
CURRENT_SYNC_STATUS=$(oc get applications.argoproj.io $APP_NAME -n $APP_NS -o jsonpath='{.status.sync.status}')
if [ "$CURRENT_SYNC_STATUS" != "Synced" ]; then
    echo -e "\n=== Out-of-Sync Resource Analysis ==="
    echo "Application is $CURRENT_SYNC_STATUS - analyzing specific resources:"
    
    # List out-of-sync resources with details
    oc get applications.argoproj.io $APP_NAME -n $APP_NS -o json | jq -r '
      .status.resources[]? |
      select(.status != "Synced") |
      "🔄 \(.kind)/\(.name) in \(.namespace // "default"):",
    ...
    echo -e "\n💡 Quick Actions for Out-of-Sync Resources:"
    echo "   - Manual sync: argocd app sync $APP_NAME"
    echo "   - Hard refresh: argocd app get $APP_NAME --hard-refresh"
    echo "   - Check specific resource: oc get <resource-type> <resource-name> -n <namespace>"
fi
