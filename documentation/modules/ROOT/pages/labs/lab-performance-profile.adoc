...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/scalability_and_performance/cnf-tuning-low-latency-nodes-with-perf-profile[Creating a performance profile]
...
----
# Get status information about the ClusterOperator/node-tuning object
...
# Verify generated TuneD profiles that are applied to your cluster node
...
# Show each MCP's nodeSelector (to see which labels they match)
oc get mcp -o jsonpath='{range .items[*]}{.metadata.name}{" => "}{.spec.nodeSelector}{"\n"}{end}'

# Expected output:
# master => {"matchLabels":{"node-role.kubernetes.io/master":""}}
# worker => {"matchLabels":{"node-role.kubernetes.io/worker":""}}
# workshop => {"matchLabels":{"node-role.kubernetes.io/workshop":""}}
...
# Explore PerformanceProfile specification options
oc explain PerformanceProfile.spec
----
...
You’ll deploy an application into the hugepages-web namespace. This app is configured to request HugePages because it performs better with them.

Alongside the app, the manifest also creates:

* the namespace hugepages-web,
* a Deployment that requests HugePages, and
* a Service and Route (so you can reach the app)
* a PerformanceProfile that tells the cluster which nodes must reserve HugePages at the kernel level.
...
oc project hugepages-web

# Check workshop MCP and node restart
...
[source,bash, subs="+macros,+attributes"]
----
oc get performanceprofile lab1-hugepages -o jsonpath='{.status.conditions}{"\n"}'
oc get performanceprofile lab1-hugepages -o jsonpath='{.spec.nodeSelector}{"\n"}'
...
[source,bash, subs="+macros,+attributes"]
----
oc get performanceprofile lab1-hugepages -o jsonpath='{.status.conditions[?(@.type=="Degraded")].message}{"\n"}'
oc get nodes -l 'node.role.kubernetes.io/performance' --no-headers
oc get nodes -l 'node-role.kubernetes.io/workshop' --no-headers

watch "oc get mcp workshop; oc get nodes; oc get pod -n hugepages-web"
----

Expected error message:
----
failed to find MCP with the node selector that matches labels "node.role.kubernetes.io/performance=fix-me"
----
====
...
**Application URL:** `http://hugepages-web-hugepages-web.apps.cluster-gxw6q.gxw6q.sandbox1933.opentlc.com`

image::hugepages-app.png[width=50%]

== Exercise 2: Troubleshooting Kernel Parameter Tuning

=== Scenario Setup

You'll deploy an application into the *conntrack-app* namespace. This application requires specific kernel parameters to function optimally, particularly for network connection tracking.

Alongside the app, the manifest also creates:

* the namespace *conntrack-app*,
* a Deployment that runs a network-intensive application,
* a Service and Route (so you can reach the app), and
* a Tuned profile that should configure kernel parameters for optimal network performance.
...
- **Is the machineConfigLabels selector correct?**
...
- This prevents the Tuned profile from being applied correctly by the Node Tuning Operator and got Degraded
...
oc get tuned -n openshift-cluster-node-tuning-operator configuration-conntrack
...
In the deployed configuration we will find errors of the type or Tuned not being to be applied because of an error in the kernel parameter configuration of the selector
...
curl http://conntrack-app-conntrack-app.apps.cluster-gxw6q.gxw6q.sandbox1933.opentlc.com
{"message":"nf_conntrack_max=8388608 OK","status":"ok"}
...
* link:https://docs.redhat.com/en/documentation/openshift_container_platform/4.19/html/scalability_and_performance/using-node-tuning-operator[Using the Node Tuning Operator]
...
* link:https://github.com/openshift/cluster-node-tuning-operator/tree/main/examples/performanceprofile/samples[PerformanceProfile samples]
