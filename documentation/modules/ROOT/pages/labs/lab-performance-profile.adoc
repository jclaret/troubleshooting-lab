...
[source,bash, subs="+macros,+attributes"]
----
# Label a worker node for the workshop MCP
...
# Add workshop label to the node
...
# Deploy the application and configuration
...
1. **What is the current status of the application pods?**
2. **Are there any errors in the pod logs or events?**
3. **What is the status of the Performance Profile and Tuned configuration?**
4. **Are there any configuration mismatches or syntax errors?**

Use the troubleshooting commands from the "Performance Profile Troubleshooting commands" section above to investigate the issue systematically.
...
- The `kernel.pid_max` parameter value is incorrectly formatted
- This prevents the Performance Profile from applying correctly
...
# Apply the corrected Tuned profile
oc apply -f solution/tuned.yaml
