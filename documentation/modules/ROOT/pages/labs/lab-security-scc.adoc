...
oc auth can-i use scc/scc-sysmodule-min --as=system:serviceaccount:module-app:module-loader
...
- Application doesnt have SCC defined
- The ServiceAccount `module-loader` doesnt exist and has no SCC permissions
- The custom SCC `scc-sysmodule-min` exists but has no users assigned to it
...
# Add SCC and SA to module-app.yaml
oc edit module-app.yaml

# Grant SCC permissions to the existing ServiceAccount
oc adm policy add-scc-to-user scc-sysmodule-min -z module-loader -n module-app

# Restart application
oc -n module-app rollout restart deploy/kernel-mod-app
