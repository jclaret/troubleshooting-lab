...
Use basic commands to correlate ArgoCD deployments with Git commit history:
...
[TIP]
====
💡 For comprehensive Git-to-ArgoCD correlation analysis, use the automated script provided in link:gitops-scripts.html[GitOps Scripts Documentation].
====
...
[TIP]
====
💡 For comprehensive sync error analysis including automated categorization, root cause detection, and detailed resource analysis, use the scripts provided in link:gitops-scripts.html[GitOps Scripts Documentation].
====
...
[TIP]
====
💡 For automated error categorization and comprehensive analysis, use the `analyze_sync_errors.sh` script from link:gitops-scripts.html[GitOps Scripts Documentation].
...
if [ "$SYNC_STATUS" != "Synced" ]; then
    ...
    if [ -n "$MISSING_RESOURCES" ]; then
        echo "⚠️  Missing Resources (may need manual creation or pruning):"
    ...
    if [ -n "$FAILED_SYNC_RESOURCES" ]; then
        echo "❌ Resources with Sync Failures:"
    ...
    echo "✅ Application is in sync - no out-of-sync resources found"
fi
...
# Example of enhanced out-of-sync analysis output:
cat << 'EXAMPLE_OUTPUT'

=== Example Enhanced Out-of-Sync Analysis Output ===

=== Out-of-Sync Resource Analysis ===
Application is out of sync - analyzing specific resources:

🔄 Deployment/web-app in my-namespace:
   Status: OutOfSync
   Health: Progressing
   Message: Deployment has minimum availability

🔄 ConfigMap/app-config in my-namespace:
   Status: Missing
   Health: Unknown
   Message: No health message

⚠️  Missing Resources (may need manual creation or pruning):
   - ConfigMap/app-config in my-namespace
   - Secret/app-secret in my-namespace

❌ Resources with Sync Failures:
   - Deployment/web-app in my-namespace

🪝 Hook Execution Failures:
   - PreSync hook: Job/migration-job - Job failed with exit code 1

=== Sync Issue Recommendations ===
1. For missing resources:
   - Check if resources were manually deleted
   - Verify namespace exists and is accessible
   - Consider using 'Replace=true' sync option for recreating resources
...
if [ -n "$POLICY_RESOURCES" ]; then
    echo "🔍 Policy-based application detected - running OCM policy analysis..."
                    ...
                    for cluster in $MANAGED_CLUSTERS; do
                        if [ -n "$cluster" ]; then
                            ...
                            if [ -n "$VIOLATION_DETAILS" ]; then
                                ...
                                echo "       Violations:"
                                echo "$VIOLATION_DETAILS" | grep -o 'violation - [^;]*' | sed 's/violation - /         🔥 /'
                                echo "       Status Summary:"
                                echo "$VIOLATION_DETAILS" | grep -o 'notification - [^;]*' | head -3 | sed 's/notification - /         ✅ /'
