...
Once connected to the bastion host, verify your OpenShift cluster authentication:
[.lines_space]
[.console-input]
[source,bash, subs="+macros,+attributes"]
----
...
If the above command fails or you need to authenticate with different credentials, login to the OpenShift API endpoint:
[.lines_space]
[.console-input]
[source,bash, subs="+macros,+attributes"]
----
# Login to the OpenShift API endpoint (replace <guid.guid> with your cluster URL)
oc login https://api.cluster-<guid>.<guid><sandbox>.opentlc.com:6443 \
  --username=kubeadmin \
...
Once you're connected to the cluster, you can access the OpenShift web console:
...
# Alternative method to get console URL
oc get route console -n openshift-console -o jsonpath='{.spec.host}'
----

**Console Access Details:**

- **URL**: Use the output from the command above
- **Username**: `kubeadmin`
- **Password**: Use the password provided in your lab credentials

**Console Features Available:**

- **Cluster Overview**: Monitor cluster health and resource usage
- **Developer Perspective**: Access to applications, deployments, and services
- **Administrator Perspective**: Full cluster management capabilities
- **Monitoring**: Built-in Prometheus and Grafana dashboards
- **Storage**: ODF console for storage management and troubleshooting

**Note**: The console provides a graphical interface for many operations that can also be performed via the `oc` CLI, making it useful for both learning and troubleshooting scenarios.
...
[NOTE]
====
The `Access Key ID` and `Secret Access Key` is provided in your lab provisioning details.
...
From the AWS console, navigate to EC2 instances. If EC2 doesn't appear under Compute on the main dashboard, use the search bar to find "EC2" and then go to "Instances".
